...
// RetryChan creates a channel on which the current time is sent up to count times.
...
// RetryChan wait and the work. For example,
//
//	  for range RetryChan(ctx, 5, 1*time.Second, 1.0) {
//		     // nothing
//	  }
//
...
//	  }
//
// will take 10 seconds (RetryChan tries to send during the sleep in the loop).
//
...
func RetryChan(ctx context.Context, count int, initial time.Duration, grow float64) <-chan time.Time {
